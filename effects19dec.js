...
async function processImage(img) {
    ...
    let isUsingGeneratedImages = false;
    
    // Add original image if valid
    ...
    if (generatedImages && Array.isArray(generatedImages)) {
        isUsingGeneratedImages = true;
        for (const generatedImg of generatedImages) {
            try {
                let processedImage;
                ...
                if (processedImage) {
                    imagesToProcess.push(processedImage);
                }
            } catch (error) {
    ...
    for (const effect of effects.filter(effect => document.getElementById(`${effect}Checkbox`).checked)) {
        ...
        for (const sourceImage of imagesToProcess) {
            const isOriginalImage = sourceImage === img;
            
            // Determine how many variations to create
            const imageCount = isUsingGeneratedImages && !isOriginalImage ? 
                             1 : // Only one variation for each generated image
                             defaultImageCount; // Default count for original image
            ...
            for (let i = 0; i < imageCount; i++) {
                try {
                    ...
                    // Get effect value based on current iteration
                    let value = getEffectValue(effect, i, imageCount);
                    ...
                    processedImages[effect].push({
                        value: value,
                        dataUrl: canvas.toDataURL(),
                        sourceType: isOriginalImage ? 'original' : 'generated',
                        sourceIndex: imagesToProcess.indexOf(sourceImage)
                    });
...
//                     sourceTitle.textContent = sourceKey.startsWith('original') ? 'Original Source' : `Generated Source ${sourceKey.split('_')[1]}`;
